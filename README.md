...
- Steam / Rust+ 登录与状态恢复
- Web 端 Steam Remote Login（隐藏任务令牌 + 本机 Chrome 插件回传）
- 服务器配对与设备绑定
- 队伍聊天指令
- 队伍聊天历史同步与网页即时回显
- 事件逻辑与系统预设
...
Web 版部署在云端时，Steam 登录通过本机 Chrome 插件桥接完成，页面不再要求用户手动输入会话码：

1. 用户先登录 Web 账号
2. Web 端创建当前账号的一次性远程登录任务
3. 用户下载安装当前任务专属的本机 Chrome 插件包
4. 插件自动打开 Rust+ 登录页
5. 插件捕获 `rustplus_auth_token`
6. 回传到云端 `/steam-bridge/callback` / `/steam-bridge/complete`
7. 云端写入当前用户自己的 `rustplus.config.json`
8. 云端继续开始配对监听与状态同步

补充说明：

- 队伍聊天页面会主动拉取历史消息，并对网页发言做本地即时回显
- 售货机事件默认会提醒“新售货机出现”与“已有售货机新增商品”
- 团队聊天过长消息会自动分条发送，并继续遵守发送间隔
